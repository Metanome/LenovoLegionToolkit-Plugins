# Pull Request

## Description
Please include a summary of the change and which issue it fixes. Include relevant motivation and context.
- Fixes # (issue number)

## Type of Change
- [ ] Bug fix (non-breaking change which fixes an issue)
- [ ] New feature (non-breaking change which adds functionality)
- [ ] Breaking change (fix or feature that would cause existing functionality to not work as expected)
- [ ] Compatibility update (adding support for new hardware)
- [ ] Documentation update

## Hardware Verification Details
Since Plugins interact directly with LLT and hardware, we require details on where this was tested.

- **Device Series:** (e.g., Legion 5 Pro, Legion 7i, LOQ)
- **Exact Model Number:** (e.g., 16ACH6)
- **BIOS Version:** (e.g., GKCN57WW)
- **Operating System:** (e.g., Windows 11 23H2)

## Testing Checklist
- [ ] I have enabled logging in **Settings** and verified the logs show no WMI/ACPI errors.
- [ ] I have verified this change on physical hardware.
- [ ] I have included a video/screenshot of the change in action (highly preferred for UI changes).
- [ ] My code follows the project's style guidelines.

## Additional Notes
Add any other context or screenshots about the pull request here.